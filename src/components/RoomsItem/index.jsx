  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <section className="relative flex flex-col justify-center h-screen w-full">
        <Image src={`/assets/${imageCard}`} layout="fill" objectFit="cover" className="w-full h-full" />
        <div className="bg-black/30 w-full absolute flex items-center justify-center" />
        <h1 className="text-2xl sm:text-4xl lg:text-5xl font-extralight font-poppins text-white absolute bottom-10 sm:bottom-20 left-0 right-0 text-center">{tipe}</h1>
      ...
      <div className="p-8 pb-20 sm:p-32">
          ...
          <div className="space-y-10 w-full sm:w-1/2 flex flex-col justify-between">
            <div>
              ...
              <div className="flex justify-center sm:justify-start">
                <button className="mt-4 px-6 py-2 bg-white border border-[#AE9578] text-[#AE9578] font-poppins">
                  Book Now
                </button>
              </div>
            </div>
        ...
        <section className="flex flex-col gap-10 mx-auto mb-40">
          <h1 className="text-2xl sm:text-4xl text-center font-libre_baskerville">Facilities</h1>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-0 gap-y-12 mt-10"> {/* Menghapus gap-x untuk kolom */}
            {facilities.map((facility, index) => (
              <div key={index} className="flex flex-col items-center gap-5 text-center font-poppins">
            ...
            <div className="grid grid-cols-2 gap-2 w-full sm:w-2/3">
              {galleryImages.slice(1, 7).map((image, index) => (
                <div key={index} className="bg-gray-300">
                  <Image src={image} layout="responsive" width={50} height={50} alt={`Gallery Image ${index + 2}`} />
                </div>
              ))}
            </div>
          </div>
                ...
                <div key={index} className="w-full sm:w-2/4 flex-shrink-0 relative p-2" onClick={() => handleImageClick(index)}>
                  <div className={`bg-white shadow-md overflow-hidden ${index % rooms.length !== currentIndex % rooms.length ? 'opacity-50 scale-90' : ''}`}>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gray-900 bg-opacity-50 p-1 w-full mx-auto">
                      <h3 className="text-lg text-center font-poppins text-white">{room.type}</h3>
                    </div>
                  </div>
                </div>
              ))}
